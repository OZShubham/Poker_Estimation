...
def create_board_id(user_id):
    ...
    return hashlib.md5(board_id_str.encode('utf-8')).hexdigest()

def create_poker_board(request):
    request_json = request.get_json(silent=True)
    if not request_json or 'team_id' not in request_json or 'user_role' not in request_json or 'user_id' not in request_json:
        return 'Status:Failure \n Error: 400 Bad Request \n Required fields : {"team_id",  "user_role", "user_id"}'
